...
suspend fun HttpClient.getAccountMonthTrendsReport(accountId: Long): List<AccountTrendDto> {
...
suspend fun HttpClient.getAccountYearTrendsReport(accountId: Long): List<AccountTrendDto> {
    this.get("$ACCOUNT_URL/$accountId/trends/year").apply {
        assertEquals(HttpStatusCode.OK, status)
        val account = Json.decodeFromString<List<AccountTrendDto>>(bodyAsText())
        return account
    }
